...
class rknn_api():
  ...
  def postproc(self, pred: np.ndarray, original_size: tuple) -> np.ndarray:
    ...
    logits_hwc = np.transpose(pred[0], (1, 2, 0))  # [171, 640, 640] → [640, 640, 171]
    logits_resized = cv2.resize(logits_hwc, (orig_w, orig_h), interpolation=cv2.INTER_LINEAR)
    logits_resized = np.transpose(logits_resized, (2, 0, 1))[np.newaxis, :, :, :]  # [1, 171, orig_h, orig_w]
    
    seg_map = np.argmax(logits_resized[0], axis=0).astype(np.uint8)  # [orig_h, orig_w]
...
if __name__ == '__main__':
  rknnApi = rknn_api(model_path='model_final_v2_coco.onnx')
  rknnApi.build()
  # rknnApi.export(output_path='model_final_v2_coco.rknn')
  rknnApi.infer(img_path='example.png')